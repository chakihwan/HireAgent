...
### 글로벌 UX
- [완료] **기능색 디자인 토큰** (info/success/warning, `@theme` 라이트/다크) — 다크모드·일관성 기반
- [완료] **브랜드 포인트 색 인디고** (`--primary`, 액션/선택 통일 · 상태색 분리)
- [완료] **한국어 폰트 Pretendard** (next/font/local self-host)
- [완료] **상단 네비 active 표시** (NavLinks usePathname)
- [완료] **공용 EmptyState** (jobs·library 빈 상태 CTA) — 일러스트는 추후
- [완료] **홈 휑함 개선** (시작 가이드 3스텝 + 폭 확대) — 라이브러리·지원관리는 추후
- [완료] **타이포 위계 + 통계 카드 밀도** (생성 사이드바 섹션 제목, 홈 카드 패딩)
- [완료] Toast 알림 통일 (`alert()` 9곳 → sonner toast, v0.7.7)
- [완료] **다크 모드** (`next-themes` + 토글 + 셸·전 페이지 토큰화) — 캔버스·SPA 안내 모달은 아래 잔여
- [ ] 로딩 스켈레톤 컴포넌트 통일
- [ ] 모바일 반응형 점검 (워크플로우 캔버스 데스크톱 우선이라 특히 필요)
- [ ] 색 하드코딩 잔여 → 토큰 (WorkflowCanvas PHASE 노드 hex 45곳 + SpaSiteGuide 내부 텍스트 — 다크 완성)
...
- [ ] F-8.4 노드 그래프 사용자 편집 (ADR-028: ✅PoC 동적빌드 → ✅on/off → ⬜순서 → ⬜완전 DAG)
