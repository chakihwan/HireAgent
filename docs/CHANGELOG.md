...
### 추가 — 대화형 마무리: 글자수 조정 · 라이브러리 저장 (ADR-031 E)

- 대화형 플로우 마지막 단계 — 초안 택1 후 **글자수 조정 · 저장** 컬럼:
  목표 대비 글자수 표시(±5%, 초과/부족 색), 본문 **직접 편집** 가능
- **"글자수 맞추기"** — 초안을 쓴 모델로 압축/확장. **판정은 Python `len()`(±5%, 최대 3회),
  LLM은 분량만**(Rule #1·ADR-001). 못 맞추면 "아직 초과/부족"으로 정직하게 보고
- **"라이브러리에 저장"** — `is_final` + `generation_metadata`(source/company/model)
- 백엔드 `POST /nodes/adjust/run`(양방향 조정 루프)
- **verify**: adjust 실측(234자→압축, status 정직 보고), 저장 실측(char_count Python 계산·메타 보존
  후 정리), tsc PASS

### 변경 — "내 경험" → 직무 적합도 지도 (ADR-032)

- **유사도 나열 → "왜 맞는가" 지도** — 뉴런 맵을 직무 요구 ↔ 경험 매칭으로 재구성:
  중심(JD) → 안쪽 고리(핵심 요구 3~5개) → 바깥 고리(내 경험). 요구↔경험을 매칭 점수로 연결
- **보강 필요(gap)** — 임계(0.4) 이상 받쳐주는 경험이 없는 요구를 **주황으로 강조**
  → "직무가 원하지만 내 경험에 약한 영역"이 한눈에 (explainable retrieval)
- **요구 추출은 LLM 미사용** — `jd_analyzer`가 이미 출력하는 `## 핵심 요구 역량` 불릿을
...
- **세련화 패스(피드백 반영)** — "충족도"→**"적합도"** 명칭, **선은 평소 흐리고 hover 시 관련 요구·
  경험만 강조**(거미줄 해소), 요구는 적합도 진행바, 경험 라벨 `owner/` 제거, 기본/크게보기 크기 ↑,
  **좌드래그 패닝**. hover 깜빡임은 `useNodesState`+스타일 in-place 패치로 해결(노드 재마운트 0)
- **verify**: coverage 실측 5요구 추출·매칭(MLOps→mnist 2위 등 차별화), 지도 31노드 + gap
  주황 렌더 스크린샷 확인, tsc·route 컴파일 PASS

### 추가 — 대화형 스튜디오: "내 경험" 신경망 (ADR-031)

- **대화형 모드** — `/generate` 빠른자동 ↔ 대화형 토글. 노드 단위 실행 API
  (`/nodes/jd-analyze·write·rag/search`)로 단계마다 **N개 모델 후보 → 사용자 택1**
  (JD분석 → 내 경험 → 초안). zustand `studio-store`로 모드 전환에도 세션 유지
- **"내 경험" 뉴런 맵** — 직무 중심 2계층 신경망(React Flow): **프로젝트=허브 · 청크=위성**.
  유사도↑일수록 중심에 가깝고 크고 밝게, 임계 0.4 자동 활성, 클릭으로 경험 켜고 끄기,
  hover 내용 미리보기 → 켜둔 경험만 작성에 인용
- **프로젝트 다양성 검색** — `search_grouped_by_project`(`ROW_NUMBER() OVER PARTITION BY` 프로젝트):
  청크 많은 한 레포가 top-k를 독식하던 문제 해결, 프로젝트별 대표 청크 균등 노출
- "RAG/유사도" 개발 용어 대신 **"내 경험"** 으로 표현 (비개발자 배려)
- **verify**: rag/search 실측 5프로젝트×5청크, 뉴런 캔버스 31노드 렌더 스크린샷 확인, tsc·route 컴파일 PASS
...
- **브랜드 포인트 색 인디고** — `--primary` 무채색 → `#4f46e5`(라이트)/`#818cf8`(다크). 액션·선택을
  primary로 통일(버튼·네비 active·항목 선택·생성 버튼), **상태색(실행중 파랑·완료 초록·경고 앰버)은 의미 유지**
  → "조작 = 인디고 / 상태 = 기능색" 역할 분리. 토큰이라 색 변경은 한 줄
- 홈 **휑함 개선** — "이렇게 시작하세요" 3스텝 카드 + `max-w-4xl→5xl` (아래 빈 공간 채움 + 온보딩)
- 통계 카드 밀도 — Card `py-4`+CardContent `p-4` 패딩 중첩 해소(`py-0`) + 아이콘 우상단 배치
- 생성 사이드바 섹션 제목 위계 — `text-xs uppercase`(한글 무의미) → `13px·zinc-800·semibold`
- RubricBars 폴리시 — 만점 표시(`/2`) + 하드코딩 색 → 토큰(success/warning/destructive)
...
- 전체 폭 카드라 좁은 사이드바에 못 넣어 끊겼던 것 → "📋 복사 방법 보기" 버튼 + 모달로 표시
- 그간 사람인 URL → "추출 불가"만 뜨고 해결책을 안 보여주던 기능 퇴행 수정
...
- 결과 카드에 막대그래프 시각화 (2=초록·1=주황·0=빨강) + 약점·개선 텍스트 분리
- "왜 이 점수인지"를 텍스트뿐 아니라 그래프로 한눈에

### 변경 — 자소서 평가를 루브릭 기반으로 개선

- 기존: "전문가인 척 1~10점 매겨라"만 지시 → 채점 기준 없음·점수 쏠림·재현성 낮음
- 5개 항목 × 0~2점 명시 루브릭 (직무적합·구체성·진정성·흐름·가독성)
- 평가 프롬프트에 JD 분석 포함 → "직무적합"을 실제 공고와 비교
- 총점은 LLM이 아니라 백엔드가 항목 점수 합산 (산수 실수 방지)
- 피드백에 항목별 점수+약점+개선 노출 (예: "직무적합 2 · 구체성 2 · 진정성 1 …")
- 파싱 실패 시 7.0 고정 → `None`("평가 실패" 명시, 프론트는 기존 null 처리)
